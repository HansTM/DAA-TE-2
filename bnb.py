...
# @profile(precision=6)
def partition_values_from_index(values, start_index, total_value, unassigned_value, test_value, ):
	...
	if start_index >= len(values):
		...
		if potential_err < best_err:
			...
			partition_values_from_index(values, start_index + 1, total_value, unassigned_value, test_value + values[start_index])
			
			# Try adding values[start_index] to set 2.
			partition_values_from_index(values, start_index + 1, total_value, unassigned_value, test_value)